...
class Start:
    def __init__(self, parent):
        ...
        self.show_job_button = Button(self.button_frame, text="Show all Job",
                                      command=lambda:self.to_history(self.job_numbers,self.customer_names,self.total_prices),
                                      bg="#1E90FF")
    ...
    def check_entries(self):
        ...
        if wof_and_tune == 0 and len(minute) == 0:
        ...
        # check distance entry can't be empty
            ...
            # check job_number entry can't be str when the wof_and_tune is chosen
                ...
                self.job_numbers.append(job_number)
                print(self.job_numbers)
                self.customer_names.append(customer_name.title())
    ...
    def calculation(self):
        ...
        if wof_and_tune == 1 and len(minute) == 0:
            ...
            self.total_prices.append(total_prices)

        elif len(minute) != 0 and wof_and_tune == 0:
            total_price = price + float(minute) * 0.8
            total_prices = "{:.2f}".format(total_price, 2)
            self.total_prices.append(total_prices)
            ...
            total_prices = "{:.2f}".format(total_price, 2)
            self.total_prices.append(total_prices)
        print(self.total_prices)


    def to_history(self,show_number,show_name,job_charge):

        History(self,show_number,show_name,job_charge)

class History:
    def __init__(self,partner,show_number, show_name, job_charge):
        ...
        self.show_job_number_label = Label(self.hist_frame, font=heading,
                                           text="Job Number:")
        self.show_job_number_label.grid(row=1,column=0, padx=0,sticky="w")

        self.show_job_number_value_label = Label(self.hist_frame,
                                      font=content, text=show_number[0])
        self.show_job_number_value_label.grid(row=1, column=1, padx=0)

        # job name label and its value
        self.show_customer_name_label = Label(self.hist_frame, font=heading,
                                           text="Customer Name:")
        self.show_customer_name_label.grid(row=2,column=0, padx=0,sticky="w")

        self.show_customer_name_value_label = Label(self.hist_frame,
                                      font=content, text=show_name[0])
        self.show_customer_name_value_label.grid(row=2, column=1, padx=0)

        # job charge label and its value
        self.show_job_charge_label = Label(self.hist_frame, font=heading,
                                           text="Job charge:")
        self.show_job_charge_label.grid(row=3,column=0, padx=0,sticky="w")

        self.show_job_charge_value_label = Label(self.hist_frame,
                                      font=content, text=job_charge[0])
        self.show_job_charge_value_label.grid(row=3, column=1, padx=0)
